<template>
  <div class="login-container">
    <div v-if="isMissingDB">
      <!-- missing entry -->
      <el-form ref="passwordForm" :model="passwordForm" :rules="setupRules" class="login-form" auto-complete="on" label-position="left">
    ...
    <div v-if="!isMissingDB && isMissingEntry">
      <div v-if="isAccountIdSet">
        <el-form ref="passwordForm" :model="passwordForm" :rules="setupRules" class="login-form" auto-complete="on" label-position="left">
          ...
          <div class="margin-clear"/>
          <el-checkbox v-model="passwordForm.email" class="title-small-nopad">{{ $t('setupSite.email_share_option') }}</el-checkbox>
          <div class="title-small-nopad-nocenter">{{ $t('setupSite.email_share_option_two') }}</div>
          <div class="title-small-nopad-nocenter">{{ $t('setupSite.email_share_option_tree') }}</div>
          <div class="margin-clear"/>
          <label class="title-small">&nbsp;</label>
          <div class="margin-clear"/>
          <label class="title-small">{{ $t('setupSite.admin_password') }}</label>
          <el-form-item prop="password">
            ...
            <el-input
              ...
              :placeholder="$t('setupSite.password')"
              name="password"
              auto-complete="on"
            />
          ...
          <div class="margin-clear"/>
          <label class="title-small">{{ $t('setupSite.serverjwt_label') }}</label>
          <el-form-item prop="serverjwt">
            <el-input
              v-model="passwordForm.serverjwt"
              :placeholder="$t('setupSite.serverjwt')"
              name="serverjwt"
              auto-complete="on"
            />
          </el-form-item>
          ...
          <div class="margin-clear"/>
          <label class="title-small">{{ $t('setupSite.clienttjwt_label') }}</label>
          <el-form-item prop="clientjwt">
            <el-input
              v-model="passwordForm.clientjwt"
              :placeholder="$t('setupSite.clientjwt')"
              name="clientjwt"
              auto-complete="on"
            />
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      setupRules: {
        ...
        serverjwt: [{ required: true, trigger: 'blur', validator: validateJWT }],
        clientjwt: [{ required: true, trigger: 'blur', validator: validateJWT }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleInitializeAccount() {
      this.loading = true
      var self = this
      this.$store.dispatch('PerformAppInitialization', self.passwordForm).then((resp) => {
        if (resp === true) {
          self.resetForm()
